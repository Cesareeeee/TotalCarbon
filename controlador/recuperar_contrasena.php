...
function enviarCorreoRecuperacion($correo, $codigo) {
    ...
    try {
        ...
        $mail->setFrom('crkendok@gmail.com', 'Total Carbon');
...
try {
    switch ($accion) {
        case 'solicitar_recuperacion':
            ...
            $consulta = "SELECT id_usuario, nombres, correo_electronico FROM usuarios WHERE correo_electronico = ? AND estado_usuario = 1";
            ...
            if ($usuario) {
                ...
                $pdo->prepare("UPDATE codigos_recuperacion SET utilizado = 1 WHERE id_usuario = ? AND utilizado = 0")->execute([$id_usuario]);
                ...
                if ($stmt_insert->execute([$id_usuario, $codigo_recuperacion, $fecha_expiracion])) {
                    ...
                    error_log("Error al insertar código");
        ...
        case 'verificar_codigo':
            ...
            $consulta = "SELECT * FROM codigos_recuperacion
                        WHERE id_usuario = ? AND codigo_recuperacion = ?
            ...
            if ($stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $pdo->prepare("UPDATE codigos_recuperacion SET utilizado = 1
                              WHERE id_usuario = ? AND codigo_recuperacion = ?")->execute([$id_usuario, $codigo]);
        ...
        case 'actualizar_contrasena':
            ...
            $consulta = "UPDATE usuarios SET contrasena = ?, actualizado_en = NOW() WHERE id_usuario = ?";

            $stmt = $pdo->prepare($consulta);
            if ($stmt->execute([$contrasena_hash, $id_usuario])) {
